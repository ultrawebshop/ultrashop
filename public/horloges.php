...
// Haal alle producten op uit de categorie "Horloge" met de zoekterm (indien aanwezig)
$query = "SELECT * FROM products WHERE category = 'Horloge' AND name LIKE :searchTerm";
...
<div class="container mt-4">
    <h1 class="text-center">Horloges</h1>

    <!-- Zoekbalk -->
    <form method="GET" action="horloges.php" class="mb-4">
    ...
    <!-- Producten -->
    <div class="row">
        <?php if (count($horloges) > 0): ?>
            <?php foreach ($horloges as $horloge): ?>
                <div class="col-md-4 mb-4">
                    <div class="product-card">
                        <a href="product_detail.php?id=<?= $horloge['id'] ?>">
                            <img src="<?= htmlspecialchars($horloge['image_path']) ?>" alt="<?= htmlspecialchars($horloge['name']) ?>">
                            <h5><?= htmlspecialchars($horloge['name']) ?></h5>
                            <p>&euro; <?= number_format($horloge['price'], 2) ?></p>
                        </a>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
            <div class="col-12">
                <p class="text-center">Geen horloges gevonden voor jouw zoekopdracht.</p>
            </div>
        <?php endif; ?>
    </div>
</div>
